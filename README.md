...
You can use Cloud Batch to run multiple instances of `cuKING` currently. After updating [`batch_job.json`](batch_job.json), run:

```sh
gcloud beta batch jobs submit cuking-gnomad-v4 --location=us-central1 --config=batch_job.json
```
...
Sharding is implemented through two parameters, `--split_factor` ($k$) and `--shard_index` ($i$), with $0 \leq i < \frac{k(k + 1)}{2}$. Make sure to update the `--split_factor` and `taskCount` setting in [`batch_job.json`](batch_job.json) accordingly.
