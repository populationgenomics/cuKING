...
__global__ void ComputeKingKernel(const uint32_t num_samples,
  ...
  const uint32_t i = index / num_samples;
  const uint32_t j = index % num_samples;
  if (i >= num_samples || i >= j) {
    return;
  }

  const uint32_t num_entries = words_per_sample / 2;
  const uint64_t offset_i = uint64_t(i) * words_per_sample;
  const uint64_t offset_j = uint64_t(j) * words_per_sample;
