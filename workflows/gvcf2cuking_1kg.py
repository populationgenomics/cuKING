...
def main():
    ...
    for gvcf_path in open('1kg_gvcfs.txt'):
        job = batch.new_job(os.path.basename(gvcf_path))
        job.image(DOCKER_IMAGE)
        job.memory('lowmem')
        job.command('export GCS_OAUTH_TOKEN=$(gcloud auth application-default print-access-token)')
        cuking_path = output_path(gvcf_path.replace('.g.vcf.gz', '.cuking'))
